...
# -*- coding: utf-8 -*-
"""
Created on Mon Mar 21 10:06:16 2022
...
class Option:
    ...
    def calc_greeks(self, verbose=False):
        ...
        try:
            ...
            self.gamma_theta = 0
        self.normalized_strike = math.log(self.strike/self.underlying_price)/((self.time_left**(1/2))*self.vol)
    ...
    def calc_theo(self, time_left=None, store=True):
        ...
        d1 = (math.log(self.underlying_price / self.strike) + time_left*(self.interest_rate-self.dividend_rate +((self.vol ** 2)/2.0)))/(self.vol * math.sqrt(time_left))
        d2 = d1 - (self.vol * math.sqrt(time_left))
        theo = None
        if self.option_type == "call":
            theo = self.underlying_price *math.exp(-1*self.dividend_rate*time_left)*norm.cdf(d1) - self.strike*math.exp(-1*self.interest_rate*time_left)*norm.cdf(d2)
        elif self.option_type == "put":
            theo = -1*self.underlying_price *math.exp(-1*self.dividend_rate*time_left)*norm.cdf(-1*d1) + self.strike*math.exp(-1*self.interest_rate*time_left)*norm.cdf(-1*d2)
        if store:
            self.d1 = d1
            self.d2 = d2
            self.theo = theo
    ...
    def calc_delta(self):
        delta = None
        if self.option_type == 'call':
            delta = math.exp(-self.dividend_rate*self.time_left)*norm.cdf(self.d1)
        elif self.option_type == 'put':
            delta = math.exp(-self.dividend_rate*self.time_left)*(norm.cdf(self.d1)-1)
        self.delta = delta
    ...
    def calc_gamma(self):
        try:
            self.gamma = math.exp(-1*self.dividend_rate*self.time_left)/(self.underlying_price*self.vol*math.sqrt(self.time_left))*math.exp(-1*(self.d1**2)/2)/math.sqrt(2*math.pi)
    ...
    def calc_theta(self, time_change=1/365):
        try:
            if self.option_type == "call":
                self.theta = (-(self.underlying_price*math.exp(-1*(self.d1**2)/2)/math.sqrt(2*math.pi)*self.vol*math.exp(-self.dividend_rate*self.time_left)/(2*math.sqrt(self.time_left)))-(self.interest_rate*self.strike*math.exp(-self.interest_rate*self.time_left)*norm.cdf(self.d2))+(self.dividend_rate*self.underlying_price*math.exp(-self.dividend_rate*self.time_left)*norm.cdf(self.d1)))/365
            else:
                self.theta = (-(self.underlying_price*math.exp(-1*(self.d1**2)/2)/math.sqrt(2*math.pi)*self.vol*math.exp(-self.dividend_rate*self.time_left)/(2*math.sqrt(self.time_left)))+(self.interest_rate*self.strike*math.exp(-self.interest_rate*self.time_left)*norm.cdf(-1*self.d2))-(self.dividend_rate*self.underlying_price*math.exp(-self.dividend_rate*self.time_left)*norm.cdf(-1*self.d1)))/365
    ...
    def calc_vega(self, vol_change=.01):
        try:
            ...
            self.vega = math.exp(-1*(self.d1**2)/2)/math.sqrt(2*math.pi)*math.exp(-1*self.dividend_rate*self.time_left)*self.underlying_price*math.sqrt(self.time_left)/100
...
class FwdBasis:
    ...
    def return_vals(self, maturities, year_diff=True):        
        ...
        if year_diff == True:
            m = 365
        for x in maturities:
            expiry_dict[x] = float(self.fitted_function(x*m)/100)
        return expiry_dict

        # else:
        #     x = pd.DataFrame([maturities, results]).T
        #     x.columns = ['days_to_maturity', 'interpolated_basis']
        #     return x
...
def getMarks():
    engine = create_engine('postgresql://postgres:ledgerprime@localhost:5432/postgres')
    query = 'SELECT * FROM "{}";'.format('marksDelta')
    marks = pd.read_sql_query(query,con=engine)
    return marks
    
def getSpotPx(currency):
    r = requests.get(url='https://deribit.com/api/v2/public/get_index_price?index_name='+currency+"_usd")
    content = json.loads(r.text)
    df = pd.Series(content["result"])
    return df

def price_options(df, currency, now, new_spot=False):
    spot =df[df['Asset']==currency]['Spot Mark'].values[0]
    if new_spot:
        if currency == "ETH" or currency == "BTC":
            spot = getSpotPx(currency.lower())['index_price']
        else:
            marks = getMarks()
            for index, row in marks.iterrows():
                if row['Ticker']==currency+"-PERP":
                    spot = row['Mark']
                    break
    for index, row in df.iterrows():
        if row['Asset'] != currency:
            continue 
        df.at[index, 'TradePX$'] = row['Px']
        df.at[index, "Spot Mark"]  = spot
        if row['Currency'] != "USD":
            df.at[index, 'TradePX$'] = row['Px']*spot
            df.at[index, "Spot Mark"]  = spot
        if row['Option Type']=='C':
            temp_call = Option(row['Mark ID'], "call", row['Strike'], row['Expiry'], row['day_diff']/365,size = row['Pos'], dividend_rate=row['Div Rate'],
                                volatility=row['IV'], underlying_price = spot)
            
            column_names , ret_data = getOptionData(temp_call)
            
            for x in range(len(column_names)):
                df.at[index, column_names[x]] = ret_data[x]
        else:
            temp_put = Option(row['Mark ID'], "put", row['Strike'], row['Expiry'], row['day_diff']/365,size = row['Pos'], dividend_rate=row['Div Rate'],
                    volatility=row['IV'], underlying_price = spot)
